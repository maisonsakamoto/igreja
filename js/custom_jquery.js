    ...
    custom.filtroGrid = function(valor, $grid, coluna) {
        // Cria um grupo de filtros que contém apenas um filtro.
        var filtergroup = new $.jqx.filter();

        // Define o valor do filtro.
        var filtervalue = valor;

        // Define o tipo de filtro como "string".
        var filtertype = 'stringfilter';

        // Define a condição de filtro como "CONTAINS" (contém).
        var filtercondition = 'CONTAINS';

        // Define o operador lógico do filtro para "or".
        var filter_or_operator = 1;

        // Cria o filtro com base no valor e na condição definidos.
        var filter = filtergroup.createfilter(filtertype, filtervalue, filtercondition);

        // Adiciona o filtro ao grupo de filtros.
        filtergroup.addfilter(filter_or_operator, filter);

        /**
         * Adiciona o filtro ao componente da tabela.
         * @param {string} dataField - O nome da coluna na qual o filtro deve ser aplicado.
         * @param {object} filterGroup - O grupo de filtros criados.
         * @param {boolean} refreshGrid - Indica se o componente da tabela deve ser atualizado após a adição do filtro.
         */
        $grid.jqxGrid('addfilter', coluna, filtergroup, true);
    ...
    /**
    * Realiza uma chamada AJAX assíncrona para um servidor usando o método HTTP POST.
    * @param {Object} obj - Os parâmetros a serem enviados para o servidor.
    * @param {string} funcao - O nome da função no servidor que será executada.
    * @param {string} view - O URL do servidor que receberá a solicitação AJAX.
    * @param {boolean} waiting - Flag para mostrar o cursor de aguardar, por padrão false.
    * @returns {Object} Um objeto jQuery AJAX que representa a solicitação AJAX assíncrona.
    * */
    custom.ajaxAsync=function(obj,funcao,view,waiting=false){                                     // FUNÇÃO AJAX
        var data = {'obj':obj,'funcao':funcao};                                             // SETA OS PARAMETROS
        if( waiting === true ){ $('*').addClass('waiting'); }
        return $.ajax({                                                                     // FAZ UM AJAX SINCRONIZADO COM A FUNCAO
            type:"POST", url:view, dataType:"json", data:data,async:true,
            complete: function(){ if( waiting === true ){ $('*').removeClass('waiting'); } }
        });
    };// FIM DA FUNÇÃO
    ...
    custom.RemovePonto = function(str){
        str= new String(toString(str));
        str  = str.replaceAll('.','');    //PARA FORMATAR PADRAO FLOAT
        return str;//parseInt(str);
    };

    /**
     * Recebe um numero float e retorna uma string com milhar
     * @param {type} String, recebe  no formato xxxxxxx.00
     * @returns {type } String formato x.xxx.xxx
     */
    custom.convertNumero=function(result){
        ...
        for (i=0;i<result.length;i++){
            if (result.toString().charAt(i)==','){ return alert('erro convertNumero =>' +result+' nao é tipo float' ); }
            if (result.toString().charAt(i)=='-'){ neg=true; }
        }
    ...
    **/
    ...
    custom.evtMaximizar = function(evt,$div){
        ...
        setTimeout(function(){
            ...
            $('#barraMin').children('div').length == 0 ? $('#barraMin').css('height','0px') : $('#barraMin').css('height','75px');
        },450);
    ...
    custom.informe = function( msg ){
        var $modal = $('<div>');
        $modal.css('line-height','1').html(msg)
...
custom.localizestrings = {
    // Símbolos e formatação de moeda
    currencysymbol: "R$",
    ...
    // Outras mensagens
    loadtext: "Carregando...",
    emptyDataString: "Nenhum dado para exibir",
    ...
    days: {
        ...
        namesShort: ["Do","Se","Te","Qu","Qu","Se","Sá"]
    },
    months: {
...
custom.confirme = function(mensagem, callback) {
    var $modal = $('<div>').text(mensagem).dialog({ title: 'Confirmação', modal: true,
        buttons: {
            'Sim': () => {
                $modal.dialog('close');
                if (typeof callback === 'function') { callback(); }
            },
            'Não': () => { $modal.dialog('close'); }
        }
    });
    return $modal;
};
...
custom.aguarde = $('<div style="text-align: center;"></div>')
                    .html('<img src="imagens/carregando.gif"/>')
                    .dialog({autoOpen: false,resizable: false,title: 'Aguarde',width:150, modal:true,draggable: false,dialogClass: "no-close",closeOnEscape: false})

custom.atualizando = $('<div style="text-align: center;"></div>')
                    .html('<img src="imagens/atualizando.gif"/ style="width: 150">')
                    .dialog({autoOpen: false,resizable: false,title: 'Atualizando',width:150, modal:true,draggable: false,dialogClass: "no-close",closeOnEscape: false})